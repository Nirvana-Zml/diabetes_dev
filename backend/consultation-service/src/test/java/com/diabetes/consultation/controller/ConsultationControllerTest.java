...
class ConsultationControllerTest {
    ...
    @Test
    void listAiDoctors_nullParams() {
        when(consultationService.listDoctors(null, null, null))
                .thenReturn(List.of());

        var result = controller.listAiDoctors(null, null, null);
    ...
    @Test
    void createSession_aiDoctorId() {
        Map<String, Object> body = Map.of("aiDoctorId", "d1");
        when(consultationService.createSession("user1", "d1"))
                .thenReturn(Map.of("sessionId", "sess_001"));
    ...
    @Test
    void sendMessage_withImage() {
        Map<String, Object> body = Map.of("content", "看一下", "imageUrl", "http://img/test.png");
        when(consultationService.sendMessage("user1", "sess_001", "看一下", "http://img/test.png"))
                .thenReturn(Map.of("messageId", "msg_001"));

        var result = controller.sendMessage("user1", "sess_001", body);

        assertEquals(200, result.code());
        verify(consultationService).sendMessage("user1", "sess_001", "看一下", "http://img/test.png");
    }
