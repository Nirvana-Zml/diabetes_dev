...
> 用户上传图片成功后，后端将 MinIO 图片 URL 及可选上下文传给工作流，由视觉大模型识别食物并返回结构化营养信息，前端用于**预填表单**；用户确认后再调用现有 `POST /api/v1/checkin/food` 完成打卡。  
...
用户在**食物打卡 → 自定义食物**流程中上传图片后，后端将 MinIO 图片 URL 及可选上下文传给 Dify 工作流，由**视觉大模型**识别食物并返回结构化营养信息，前端用于**预填表单**（名称、分类、每克热量、是否液体、建议食用量等），用户确认后再调用 `POST /api/v1/checkin/food` 完成打卡。
...
| 入参布局 | **flat**（开始节点 6 变量平铺，与打卡行为分析工作流一致） |
...
| 食物识别 | LLM（Vision） | 模型需支持图片 URL 输入（如 GPT-4o / Qwen-VL / DeepSeek-VL）；开启 **Structured Output**，Schema 见 §5.2 |
...
- 核心营养字段（`food_name`、`category_id`、`calories_per_gram`、`is_liquid`、`ml_to_g_ratio`）与 `USER_FOOD_PRESETS` **对齐**，便于自定义打卡后写入用户模板（见 §10）
...
自定义食物打卡成功后，`UserCustomPresetService.saveFoodTemplate` 会将核心营养字段写入 `USER_FOOD_PRESETS`（见 §10）。
...
## 10. 与 `USER_FOOD_PRESETS` 表字段对照

`db/migration/migration_user_custom_presets_v1.sql` 定义的 `USER_FOOD_PRESETS` 用于保存用户**自定义食物模板**（打卡页「自定义」分类复用），与工作流返回数据的对应关系如下。

### 10.1 核心营养字段（一一对应，匹配）

| 表字段 | 工作流返回字段 | 类型转换 | 说明 |
|--------|----------------|----------|------|
| `CATEGORY_ID` | `category_id` | string → VARCHAR(32) | 直接映射；须为 `food_categories` 中的合法 ID |
| `FOOD_NAME` | `food_name` | string → VARCHAR(100) | 直接映射；表上有 `(USER_ID, FOOD_NAME, DEL_FLAG)` 唯一约束 |
| `CALORIES_PER_GRAM` | `calories_per_gram` | number → DECIMAL(8,4) | 直接映射；精度足够（常见 0.2~3.0 kcal/g） |
| `IS_LIQUID` | `is_liquid` | boolean → TINYINT | `true`→1，`false`→0；与 `FOOD_PRESETS.IS_LIQUID` 语义一致 |
| `ML_TO_G_RATIO` | `ml_to_g_ratio` | number → DECIMAL(6,4) | 直接映射；非液体默认 1.0 |

**结论：工作流返回的 5 个核心营养字段与 `USER_FOOD_PRESETS` 设计完全匹配**，用户确认自定义打卡后，`UserCustomPresetService.saveFoodTemplate` 可直接从预填表单写入上述列。

### 10.2 非工作流来源字段（后端自行填充）

| 表字段 | 来源 | 说明 |
|--------|------|------|
| `FOOD_ID` | 后端生成 `uf_` 前缀 ID | 新建时 `IdGenerator.nextId("uf_")`；同名更新时复用已有 ID |
| `USER_ID` | JWT | 不从工作流返回 |
| `IMAGE_OBJECT_KEY` | MinIO 上传接口 | 来自 `POST /checkin/upload-image` 的 `objectKey`，**非**工作流输出 |
| `DEL_FLAG` / `CREATED_AT` / `UPDATED_AT` | 数据库默认 | 运维字段 |

### 10.3 工作流返回但不入 `USER_FOOD_PRESETS` 的字段（符合设计）

| 工作流字段 | 不入模板原因 | 实际去向 |
|------------|--------------|----------|
| `category_name` | 冗余展示字段 | 打卡时写入 `CHECKIN_DIET_DETAILS.CATEGORY_NAME`；模板侧可通过 `CATEGORY_ID` 关联查询 |
| `suggested_input_unit` | 每次打卡份量不同 | 预填 `FoodCheckinRequest.inputUnit` → `CHECKIN_DIET_DETAILS` |
| `suggested_input_amount` | 同上 | 预填 `FoodCheckinRequest.inputAmount` |
| `suggested_grams` | 同上 | 写入 `CHECKIN_DIET_DETAILS.GRAMS` |
| `suggested_total_calories` | 同上 | 写入 `CHECKIN_DIET_DETAILS.TOTAL_CALORIES` |
| `matched_food_id` / `source_type` | 选用预设 vs 自定义的决策 | 仅影响前端模式切换与 `FoodCheckinRequest.sourceType` |
| `confidence` / `gi_level` / `nutrition_tip` / `recognition_summary` / `items` | AI 辅助展示 | 前端展示，不入库 |
| `has_error` / `error_message` | 识别失败 | 前端提示，不写模板 |

### 10.4 数据流小结

```mermaid
flowchart LR
    A[工作流 food_recognition] --> B[前端预填表单]
    B --> C[用户确认份量]
    C --> D[POST /checkin/food]
    D --> E[CHECKIN_DIET_DETAILS]
    D --> F[USER_FOOD_PRESETS]
    G[MinIO objectKey] --> D
    G --> F
```

- **模板表**只存可复用的食物定义（名称 + 分类 + 营养密度 + 图片），不存每次的食用量。
- **打卡明细**存当次餐次、份量、总热量等会话数据。
- 二者分工与 `migration_user_custom_presets_v1.sql` 表结构一致，**无需为工作流返回数据增删 `USER_FOOD_PRESETS` 列**。
