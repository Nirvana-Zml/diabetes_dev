...
### 4.0 工作流结束节点输出变量（与 Dify 编排一致）

| 字段 | 类型 | 来源节点 | 说明 |
|------|------|----------|------|
| `valid` | boolean | 代码执行 | 输入校验是否通过 |
| `message` | string | 代码执行 | 校验说明（通过/失败提示） |
| `error_message` | string | 代码执行 | 校验失败时的用户可见错误 |
| `error_type` | string | 代码执行 | 错误类型标识 |
| `text` | string | LLM | 校验通过后的 Markdown 科普回答 |

后端在 SSE 中处理：`text_chunk` / `message.answer` 流式输出 `text`；`workflow_finished.outputs` 中 `valid=false` 时返回 `error_message`（或 `message`）。
